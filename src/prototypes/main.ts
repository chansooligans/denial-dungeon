...
const prototypes: Prototype[] = [
...

  {
    ...
    subtitle: '@ L7 — high-cost ICU stay trips the stoploss',
    archetype: 'Reckoner',
    carc: 'contractual underpayment',
    level: 7,
    status: 'shipped',
    verbs: 'TRIP + RECALCULATE + APPEAL',
    testing: 'Lucia Romero, 22-day ICU sepsis stay (DRG 871). $312k charges, $48k case rate paid. Anthem contract has a stoploss provision: charges > 4× case rate ($192k threshold) flips payment to 65% of charges ($202,800). Player walks 4 trip statements (math + mutual-exclusivity), picks the right formula (4 candidates; 65% is correct, 75% outlier and 100% billed are decoys), and files the appeal with the right shortfall ($154,800) on the right citation. First Case where the math is the puzzle, not the lookup.',
